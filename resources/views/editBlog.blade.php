...
<div id="edit_photo" class="edit-photo-modal modal">
    ...
    <form  class="drag-drop" method="POST" role="form" enctype="multipart/form-data" action="{{ url('/editBlog') }}" novalidate>
    	{{ csrf_field() }}

    	@foreach($blogs as $blog_var)
        <input type="file" name="bookcover" value="{{ $blog_var->HeadImg }}" id="bookcover" />
        <div class="drag-files-label">
            <br>Choose Blog Head image
        </div>
        
        <input type="hidden" name="blogId" value="{{ $blog_var->blogId }}" id="blogId">
        <input type="hidden" name="headData" value="{{ $blog_var->Title }}" id="headData">
        <input type="hidden" name="contData" value="{{ $blog_var->Content }}" id="contData">
        <input type="hidden" name="BlogCat" value="{{ $blog_var->BlogCat }}" id="BlogCat">
        @endforeach
...
@endforeach
 ...
 <br><br><br>
 <button onclick="saveBlog()" class="button">Update Blog</button>
...
<script>
	...
	$(document).ready(function(){
		...
		$("#heading").text( $("#headData").val() );
		$("#content").text( $("#contData").val() );
		$("#blog_category").val( $("#BlogCat").val() );
